...
update_dimension_tables_queries = [
    datasource_table_insert,
    region_table_insert,
    time_table_insert,
]
update_fact_tables_queries = [
    fact_delta_trips_table_insert,
    fact_trips_table_insert,
    fact_group_trips_table_insert,
]
...
"""
with week_data as(
select et.week, r.region, sum(t.number_of_trips) number_of_trips from trips.fact_group_trips t
inner join trips.dim_region r
on r.region_id = t.region_id
inner join trips.dim_event_time et
on et.event_timestamp = t.event_date
group by et.week, r.region)
select region, avg(number_of_trips) from week_data
group by region
...
select distinct region from trips.dim_datasource dd
